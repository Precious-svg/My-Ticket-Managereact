...
export default function TicketCard({ ticket, onAdvance, onEdit, onDelete }) {
  return (
    <article aria-labelledby={`t-${ticket.id}`}>
      <Card className="rounded-lg shadow-xl px-6 py-4 min-h-[200px]" bgColor={ticket.status === "open" ? "bg-green-300" : ticket.status === "closed" ? "bg-gray-400" : "bg-amber-500"}>
        ...
        <div className="kv" style={{marginTop:'0.75rem'}}>
           ...
           <div className="my-3 flex gap-4 justify-between items-center">
              <Button onClick={() => onAdvance(ticket)} color="bg-emerald-200" textColor="text-stone-900" className="border-1 border-white cursor-pointer">Advance</Button>
              <Button onClick={() => { console.log("Delete clicked for:"); onDelete()}}   color="bg-red-600" className="cursor-pointer">Delete</Button>
              <Button onClick={() => onEdit(ticket)}  color="bg-sky-400"  textColor="text-stone-600" className="border-1 border-white cursor-pointer">Edit</Button>
